"""NaviLink cloud API client for Navien water heaters.

This module is part of the willh6/hass_navien_water_heater integration, a
stability-focused fork of the original work by nikshriv and bakerkj. The
networking/protocol logic (REST sign-in, AWS IoT MQTT topics and payloads) is
preserved from the original; the reconnect/lifecycle handling has been
rewritten in v2.0.0 to fix a file-descriptor leak that could crash Home
Assistant Core. See CHANGELOG.md for details.
"""
import asyncio
import enum
import json
...
# Reconnect backoff (seconds): starts at MIN, doubles each consecutive failure,
# capped at MAX. Resets to MIN after a successful connection. This replaces the
# original fixed 15s retry, which—combined with the FD leak—let failures pile up
# faster than they could drain.
RECONNECT_BACKOFF_MIN = 15
...
class NavilinkConnect():
    ...
    def __init__(self, userId, passwd, device_index = 0, polling_interval = 15, aws_cert_path = "AmazonRootCA1.pem", subscribe_all_topics=False):
        ...
        self.last_poll = None
        # v2.0.0: guards against overlapping reconnect loops (the original FD
        # leak) and tracks current backoff delay.
        ...
        if self.polling_interval > 0:
            ...
            while not self.connected and valid_user and not self.shutting_down:
                try:
                    ...
                    # v2.0.0: a failed login()/_connect_aws_mqtt() may have left
                    # a half-built MQTT client behind. Release it before retrying
                    # so startup failures don't leak FDs the way the old code did.
                    await self._cleanup_client()
                    await asyncio.sleep(self._backoff)
                    self._backoff = min(self._backoff * 2, RECONNECT_BACKOFF_MAX)
                else:
                    # v2.0.0: connection succeeded—reset backoff to the floor.
                    self._backoff = RECONNECT_BACKOFF_MIN
    ...
    async def _start(self):
        if not self.shutting_down:
            ...
            for task in pending:
                task.cancel()
            # v2.0.0: always tear down the existing MQTT client before
            # reconnecting. The original left the old AWSIoTPythonSDK client
            # (and its sockets) dangling on every cycle, leaking file
            # descriptors until HA Core ran out and crashed.
            await self._cleanup_client()
            if not self.shutting_down:
                self.connected = False
                # v2.0.0: a single guarded reconnect with exponential backoff,
                # instead of unconditionally spawning a new start() task (which
                # could run concurrently with this one and multiply the leak).
                await self._schedule_reconnect()

    async def _cleanup_client(self):
        """Disconnect and release the AWS IoT MQTT client and its sockets."""
        client = self.client
        self.client = None
        if client is None:
            return
        try:
            ...
            # The client is frequently already dead at this point; the original
    ...
    async def _schedule_reconnect(self):
        ...
        if self._reconnecting or self.shutting_down:
            return
        self._reconnecting = True
        try:
            delay = self._backoff
            _LOGGER.warning(
                "Connection to AWS IoT NaviLink server reset, reconnecting in %s seconds",
                delay,
            )
            await asyncio.sleep(delay)
            # Grow backoff for the *next* failure; reset happens in start() on success.
            self._backoff = min(self._backoff * 2, RECONNECT_BACKOFF_MAX)
            if not self.shutting_down:
                await self.start()
        finally:
    ...
    async def _connect_aws_mqtt(self):
        ...
        if accessKeyId and secretKey and sessionToken:
            ...
            # v2.0.0: configureCredentials() opens and reads the CA cert file
            # synchronously. Run it in an executor so it doesn't block HA's event
            # loop (Home Assistant logs this as a blocking-call warning otherwise).
    ...
    async def disconnect(self,shutting_down=True):
        # v2.0.0: always release the client on a shutting-down disconnect (e.g.
        # config-entry unload/reload), not only when currently connected, so a
        # client built during a failed startup can't linger.
        if shutting_down:
